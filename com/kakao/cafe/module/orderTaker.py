...
def takeOrder(self):  #손님에게 주문을 받는 함수 takeOrder
    ...
    try:
        self.__ordertoCustomer = input().split()  #문자열 리스트로 받는 input을 만들어준다.
        self.__ordertoCustomer = list(map(
            int, self.__ordertoCustomer))  #문자열 리스트를 정수형으로 바꿨을때 Error가 나면 안됨.
        for length in len(self.__ordertoCustomer):
            if (self.__ordertoCustomer >= 1 and self.__ordertoCustomer <=
                    30):  #메뉴 숫자가 30까지이기때문에 그사이에 안들어가면 오류남.
                pass
    ...
    for Length in range(
            self.__ordertoCustomer):  #손님에게 주문받은 ordertoCustmore만큼 반복
        #손님이 주문한 번호를 간략하게 바꿔놓는다.
        number = self.__ordertoCustomer[Length]
        #에스프레소
        if (number == 1):
...
def addallPrice(self, price: int) -> list:
    if (price <= 8):  # 1~8 에스프레스
        self.__allPrice = self.__allPrice + self.__orderList.append(
            MenuPrinter.getEspresso[price].getPrice() *
            self.__orderList[len - 1])
    elif (price <= 11):
        price -= 9  # 9~11 에이드
        self.__allPrice = self.__allPrice + self.__orderList.append(
            MenuPrinter.getAde[price].getPrice() * self.__orderList[len - 1])
    elif (price <= 14):
        price -= 12  # 12~14 스무디
        self.__allPrice = self.__allPrice + self.__orderList.append(
        ...
        price -= 15  # 15~ 23 티
        self.__allPrice = self.__allPrice + self.__orderList.append(
            MenuPrinter.getTea[price].getPrice() * self.__orderList[len - 1])
    else:
        price -= 24  #23 ~ 30 디저트
        self.__allPrice = self.__allPrice + self.__orderList.append(
...
def askGreenTea(self):
    try:
        ...
        if (self.addGreenTea == True):
            ...
            self.__allPrice = self.__allPrice + (int(self.addGreenTea) * 300)
...
def askStrawberry(self):
    try:
        ...
        if (self.addStrawberry == True):
            ...
            self.__allPrice = self.__allPrice + (int(self.addStrawberry) * 500)
...
def askPineapple(self):
    try:
        ...
        if (self.addPineapple == True):
            ...
            self.__allPrice = self.__allPrice + (int(self.addPineapple) * 500)
...
def askChamomileTea(self):
    try:
        ...
        if (self.addChamomileTea == True):
            ...
            self.__allPrice = self.__allPrice + (int(self.addChamomileTea) *
                                                 500)
    ...
    try:
        ...
        if (self.BlackTea == True):
            ...
            if (self.BlackTea == True):  #블랙티 추가
                ...
                self.__allPrice = self.__allPrice + (int(self.addBlackTea) *
                                                     500)
...
def askPeppermintTea(self):
    try:
        ...
        if (self.addPeppermintTea == True):
            ...
            self.__allPrice = self.__allPrice + (int(self.addPeppermintTea) *
                                                 500)
...
def askRooibosTea(self):
    try:
        ...
        if (self.addRooibosTea == True):
            ...
            self.__allPrice = self.__allPrice + (int(self.addRooibosTea) * 700)
...
def askWaffle(self):
    try:
        ...
        if (self.addWaffle == True):
            ...
            self.__allPrice = self.__allPrice + (int(self.addWaffle) * 1000)
...
def askFruitsBlueberry(self):
    try:
        ...
        if (self.addFruitsBlueberry == True):
            ...
            self.__allPrice = self.__allPrice + (int(self.addFruitsBlueberry) *
                                                 1000)
...
def askIceCream(self):
    try:
        ...
        if (self.addIceCream == True):
            ...
            self.__allPrice = self.__allPrice + (int(self.addIceCream) * 500)
...
def askRedVelvetPowder(self):
    try:
        ...
        if (self.addRedVelvetPowder == True):
            ...
            self.__allPrice = self.__allPrice + (int(self.addRedVelvetPowder) *
                                                 500)
...
def askChocolatePowder(self):
    try:
        ...
        if (self.addChocolatePowder == True):
            ...
            self.__allPrice = self.__allPrice + int(self.addChocolatePowder)
