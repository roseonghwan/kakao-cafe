...
class PaymentManager:
    def __init__(self):
        ...
        self.__cardBalance = 0  #카드 잔액(set함수 필요)
        self.__cash = 0  #소유 현금(set함수 필요)(get함수 필요)
        self.__gifticon = 0  # 기프티콘 가치(set함수 필요)
        self.__change = 0  #거스름돈(get함수 필요)
        self.__point = 0  #적립 포인트(set함수 필요)(get함수 필요)
        ...
        if self.__cardBalance >= self.totalPrice:
            try:
                ...
                if select == 1:  # 포인트를 사용해 할인되어 결제하는 경우
                    self.__cardBalance -= self.totalPrice - self.getPoint()
                    ...
                    print("카드결제가 완료되었습니다.")
                    self.__point += self.getPoint(
                    ) + self.totalPrice * 0.03  # 포인트를 사용하지 않고 결제한 경우 가격의 3%가 적립되어 point에 저장됨
    ...
    def gifticonPay(self):  # 기프티콘 결제
        if self.__gifticon >= self.totalPrice:
            print("기프티콘을 사용하셨습니다.")
        else:
            ...
            try:
                ...
                if select == 1:  # 남은 차액을 카드로 결제 진행
                    if self.__cardBalance >= self.totalPrice - self.__gifticon:
                        self.__cardBalance -= self.totalPrice - self.__gifticon
                        print("차액이 카드결제로 완료되었습니다.")
                    else:
                        print("잔액이 부족합니다.")
                else:  # 남은 차액을 현금으로 결제 진행
                    if self.__cash >= self.totalPrice - self.__gifticon:
                        self.__change = self.__cash - (
                            self.totalPrice - self.__gifticon
                        )  # 남은 차액을 현금으로 결제할 때 나온 거스름돈 계산
                        print("받은 금액은 " + str(self.getCash()) + "원 입니다.")
                        print("거스름돈은 " + str(self.getChange()) + "원 입니다.")
    ...
    def setGifticon(
        self, gifticon: int):  # 처음 기프티콘의 가치를 얼마로 설정할지 필요한  gifticon의 setter
        self.__gifticon = gifticon

    def setPoint(self, point: int):  # 처음 포인트를 얼마로 설정할지 필요한 point의 setter
        self.__point = point
    ...
    def getGifticon(self):
        return self.__gifticon