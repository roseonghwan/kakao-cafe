...
import time  #시간 출력
'''
def takeOrder(self, menu: cafeMenu):
    try:
        self.__orderList.append(menuList[menu])
'''


class OrderTaker(MenuPrinter):
    def __init__(self, ):
        self.menuPrinter = MenuPrinter()  # 인스턴스화 X
        ...
        self.__addList['addStrawberry'].append(
            self.menuPrinter.StrawberryAde.addStrawberry(
                self.mStrawberryAde(), self._amount))
        self.__addList['addBelgianWaffle'] = list()
        self.__addList['addBelgianWaffle'].append(
            self.menuPrinter.BelgianWaffle.addWaffle(
                self.menuPrinter.BelgianWaffle(),
        ...
        # addnameList를 추가한 이유는 addWaffle과 같이 상속된 클래스들이 많은 경우 구별을 해주기 위해서다.
        # 나중에 나눌때 addWaffle.append()에서 그 종류를 더해주면 나눌때 더욱 편리할듯.
        # addList의 addWaffle도 addIceWaffle 이런식으로 분류해줘야 할듯
        '''
    ...
    def orderTaker(self, _order) -> list:
        ...
        #self.time = self.getOrdertime()  #시간을 측정한다. 테스트 코드 작성할 때 시간 append 하기 위해서
